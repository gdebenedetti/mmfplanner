...
/**
 * This class is an adapter between project properties GUI and project properties model. It listens to changes
 * in both the model and the GUI
 */
public class ProjectPropertiesAdapter implements PropertyChangeListener, FocusListener, ActionListener {
	 ...
	 */
	public ProjectPropertiesAdapter(JFormattedTextField periodsTextField, JTextField interestRateTextField,
			JTextField weightedfactorTextField, JTextField projectNameTextField,
		...
		this.weightedfactorTextField = weightedfactorTextField;
		...
		weightedfactorTextField.addFocusListener(this);
		weightedfactorTextField.addActionListener(this);
	...
	public void setModel(Project project) {
		...
		if (null != project) {
			...
			interestRateTextField.setText(interestFormat(project.getInterestRate()));
			weightedfactorTextField.setText(interestFormat(project.getWeightFactor()));
	...
	private void updateModel(Object source) {
		if (source == periodsTextField) {
		...
		}  else if (source == weightedfactorTextField) {
			String s = weightedfactorTextField.getText();
			Pattern p = Pattern.compile("([0-9]*)(?:[\\.,]([0-9]+))?.*");
			Matcher m = p.matcher(s);
			...
			if (m.matches()) {
				...
				double value = Double.parseDouble(s) / 100.0;
				project.setWeightFactor(value);
			}

			updateComponent(Project.EVENT_WEIGHTING_FACTOR);
			...
			try {
				...
				int value = ((Number) maxMmfsPerPeriodTextField.getValue()).intValue();
	...
	private void updateComponent(String event) {
		if (null != project) {
			if ((null == event) || Project.EVENT_PERIODS.equals(event)) {
				periodsTextField.setValue(project.getPeriods());
			} else if ((null == event) || Project.EVENT_INTEREST_RATE.equals(event)) {
				interestRateTextField.setText(interestFormat(project.getInterestRate()));
			} else if ((null == event) || Project.EVENT_WEIGHTING_FACTOR.equals(event)) {
				weightedfactorTextField.setText(interestFormat(project.getWeightFactor()));
