...
/**
 * Contains all information related to an IFM Project. This will handle involved MMFs, data-consistency,
 * calculations that include more than one MMF. If will also forward any events from MMFs and Categories so
 * that it is easier for observers to observe only the Project object and not handle which MMFs and Categories
...
public class Project implements PropertyChangeListener {
	...
	/**
	 * Sets the number of periods and fires the event EVENT_PERIODS. Must be greater than 0. Will not make any
	 * changes to the revenue data of MMFs. That means MMF will remember revenue data that is entered for
	 * periods beyond this setting.
	 * 
	 * @throws MmfException
	 */
	...
	/**
	 * Sets the interest rate per period and fires the event EVENT_INTEREST_RATE. The value should be
	 * absolute, not in percent. (i.e. 12% = 0.12)
	 */
	...
	/**
	 * Adds the category to the list of categories and fires the EVENT_CATEGORIES event.
	 */
	public void addCategory(Category category) {
		if (categories.indexOf(category) >= 0) {
			throw new IllegalArgumentException("This category already exists: " + category);
		}
	...
	/**
	 * @return a copy of the category list. Modifications to this list will not have any effect on the
	 *         category list.
	 */
	...
	/**
	 * Removes the category from the list of categories and fires the EVENT_CATEGORIES event.
	 */
	...
	/**
	 * @param maxMmfsPerPeriod
	 *            the maxMMFsPerPeriod to set
	 * @throws MmfException
	 */
	public void setMaxMmfsPerPeriod(int maxMmfsPerPeriod) throws MmfException {
		if (maxMmfsPerPeriod < 0) {
			throw new MmfException("Invalid maxMMFsPerPeriod: " + maxMmfsPerPeriod);
		}
		int oldValue = this.maxMmfsPerPeriod;
		this.maxMmfsPerPeriod = maxMmfsPerPeriod;
		changeSupport.firePropertyChange(EVENT_MAX_MMFS, oldValue, maxMmfsPerPeriod);
	}

	/**
	 * Adds the mmf to the list of mmfs and fires the EVENT_MMFS event.
	 * 
	 * If the MMF contains an invalid or duplicate id it will be reassigned a new id.
	 */
	public void add(Mmf mmf) {
		if (mmfs.indexOf(mmf) >= 0) {
			throw new IllegalArgumentException("This MMF already exists: " + mmf);
		}
		...
		if (!isValidId(mmf.getId())) {
			try {
				...
				throw new IllegalArgumentException("isValidId() != isValidId()", e);
			}
		}
		mmf.setProject(this);
		try {
	...
	/**
	 * Checks if the given id is valid and has no duplicates in the current project
	 * 
	...
	/**
	 * @return the id that should be used for the next MMF that is added. The value is not increased until the
	 *         next MMF is actually added.
	 */
	...
	/**
	 * @return An array of SANPV arrays. The first element is the SANPV array of the first MMF, etc.
	 */
	...
	/**
	 * Is called whenever there is a change in a child MMF or Category. Project does not directly use this,
	 * but forwards all events to the PropertyChangeListeners of this project.
	 */
	...
	/**
	 * Add a PropertyChangeListener to be notified of changes to this object or child objects (MMFs and
	 * Categories)
	 */
